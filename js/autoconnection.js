...
var user;
var ccf = true;
var skipCountrer = false;

//
var _data = [];
var _counter = null;
var _is_have_iframe = false;
var _iframe = null;


var _class = ".entity-result__actions button";





var dr1 = localStorage.delete_requests;
...
function checkcurrentnext() {
	if (s.feature == 1 && $(_class).length) {

		for (icounter = i; icounter < $(_class).length; icounter++) {
			var btn_text_temp = $(_class).eq(icounter).text().trim();
	...
	if (s.feature == 2 && $(_class).length) {

		for (icounter = i; icounter < $(_class).length; icounter++) {
			if ($(_class).eq(icounter).text().match(/Message|Nachricht/gi)) {
				i = icounter;
				is_current_available = 1;
				break;
			}
	...
	if (s.feature == 3 && $(_class).length) {

		if (i < $(_class).length) {
			is_current_available = 1;
			is_next_available = $(_class).length - 1;
			if (is_next_available > i) {
				is_next_available = 1;
			}
			else {
...
var is_current_available = 0;
var is_next_available = 0;
	...
	if (s.pause == false) {
			...
			setTimeout(() => {
				...
				try {
					if (s.feature == 13) {
						...
						var pp = $(".message-anywhere-button").not("[lac='true']").eq(0).closest(".groups-members-list__typeahead-result").eq(0);
						var url = "https://www.linkedin.com" + pp[0].getElementsByTagName("a")[0].getAttribute("href");
						var username = pp[0].getElementsByTagName("a")[0].getAttribute("href").replace(/\/in\/|\//gi, "");

						var fullname = pp.find(".artdeco-entity-lockup__title").text().trim();

						var fname = fullname.split(" ")[0];
						var lname = fullname.split(" ")[1];
						var title = pp.find(".artdeco-entity-lockup__subtitle").text().trim();
						...
						if ((s.feature != 1) || (is_current_available == 1)) {
							...
							var fullname = pp.find(".actor-name").text();

							var fname = fullname.split(" ")[0];
							var lname = fullname.split(" ")[1];
							var title = pp.find(".subline-level-1").text();
		...
		if (!window.location.href.match(/lacontrol=endrose/gi)) {
			...
			chrome.storage.local.get("counterConnection", function (x) {
					...
					function send_message() {
						...
						setTimeout(() => {
							try {
								...
								$("*[data-control-name='overlay.close_conversation_window']", _iframe.contentWindow.document).each(function () {
									$(this).trigger("click");
								})
								...
								setTimeout(() => {
									...
									setTimeout(function () {
										try {
											...
											var name = _iframe.contentWindow.document.querySelector(".pv-top-card--list .t-24").innerText.split(" ")[0];
											var n = s.note;
											n = n.replace(/\[+firstname+\]/g, name);

											var t = $(".msg-overlay-conversation-bubble", _iframe.contentWindow.document).not(".msg-overlay-conversation-bubble--is-minimized");
											...
											if (t.prevObject[0].innerText.indexOf(n) === -1) {
												...
												if (textarea.length) {
													...
													setTimeout(function () {
														...
														t.find('.msg-form__send-button').attr("disabled", false);
														t.find('.msg-form__send-button').trigger("click");

														setTimeout(function () {
															try {
																t.find(".artdeco-modal__dismiss").click();
															} catch (error) {
																console.log(error);
															}
															t.find("li-icon[type='cancel-icon']").parent().click();
															deduct();
														}, 700);
													}, 500);
												}
						...
						if (document.body.contains(see_all_followers)) {
							...
							setTimeout(() => {
								...
								for (var i = 0; i < followers.length; i++) {
									...
									let f_name = followers[i].querySelector(".artdeco-entity-lockup__title").innerText.split(" ");
									let title = followers[i].querySelector(".artdeco-entity-lockup__caption").innerText;
									let name = f_name[0];
									f_name.shift();
									let surname = f_name.toString().replaceAll(",", " ");
				...
				if (s.feature == 3) {
					...
					setTimeout(() => {
						...
						var isnextpage = 0;
						if ($(".artdeco-pagination__button--next:not(:disabled)").length) {
			...
			setTimeout(() => {
				...
				if (is_current_available == 1 && s.feature == 1 && $(_class).length) {

					var btn_text = $(_class).eq(i).text().trim();
					if (btn_text.match(/connect|vernetzen/gi)) {

						chrome.storage.local.get("requests_data", function (r) {
							var d = r.requests_data;
							var uu = new Array();
					...
					var umatch = delete_requests.filter(function (x, y) {
						return x.username == username;
					})
					$("html").animate({
						scrollTop: $(_class).eq(i).offset().top - 300
					}, 1500, function () {
						if (umatch.length == 0 && !btn_text.match(/Invite Sent|InMail|Einladung gesendet/gi)) {

							$(_class).eq(i).trigger("click");

							var name = $(_class).eq(i).parents(".entity-result").find("span.entity-result__title-line.flex-shrink-1.entity-result__title-text--black > span > a > span > span:nth-child(1)").text();

							var full_name = name.split(" ");;
							name = name.split(" ")[0];

							var profile_link = $(_class).eq(i).parents(".entity-result").find("a.app-aware-link").attr("href");

							if (profile_link.indexOf("headless?origin=FACETED_SEARCH") !== -1) {
								profile_link = "hidden";
							}

							var title = $(_class).eq(i).parents(".entity-result").find(".entity-result__primary-subtitle").text();
							var data = JSON.parse(localStorage.data);

							full_name.shift();

							var last_name = full_name.toString().replaceAll(",", " ");
								...
								if (!modalDialog.find($(".artdeco-button--secondary"))[0].hasAttribute("disabled")) {
									...
									setTimeout(() => {
										...
										setTimeout(() => {
											...
											modalDialog.find($(".artdeco-button--primary")).removeAttr("disabled");
											modalDialog.find($(".artdeco-button--primary")).click();
											console.log("");
											deduct();
										}, 1000)
									}, 1000)
								} else {
									modalDialog.find($(".artdeco-modal__dismiss")).click();
									deduct();
								}
								$(_class).eq(i).text("Invite Sent");
								$(_class).eq(i).css({
									color: "grey",
									fontSize: "14px"
								});
			...
			setTimeout(() => {
				...
				// Auto Messaging
				if (is_current_available == 1 && s.feature == 2 && $(_class).length) {
					...
					$("html").animate({
						...
						console.log($(_class).eq(i).text().match(/Message|Nachricht/gi));
						if ($(_class).eq(i).text().match(/Message|Nachricht/gi)) {

							$("*[data-control-name='overlay.close_conversation_window']").each(function () {
								$(this).trigger("click");
							})

							$(_class).eq(i).trigger("click");

							var t = $(".msg-overlay-conversation-bubble").not(".msg-overlay-conversation-bubble--is-minimized");
							...
							setTimeout(function () {
								try {
									...
									if (t.prevObject[0].innerText.indexOf(n) === -1) {
										...
										var ntext = n.replace(/\s+/g, ' ').trim();
										//ntext = ntext.replace(/\?/gi,'\\?');

										ntext = escapeRegExp(ntext);
										var re = new RegExp(ntext, "gi");

										var ttext = t.text().replace(/\s+/g, ' ').trim();

										if (!ttext.match(re)) {

											t.find('.msg-form__contenteditable p').text(n);
											...
											setTimeout(function () {
												try {
													t.find('.msg-form__send-button').removeAttr("disabled");
			...
			setTimeout(() => {
				...
				// Auto profile view and endorse
				if (is_current_available == 1 && s.feature == 3 && $(_class).length) {
					console.log(111111)
					...
					var t = $(".reusable-search__result-container").eq(i);
					console.log(t)
					//var t = $(".search-result.search-result__occluded-item").eq(i)[0];debugger
					if (typeof $(t).find("a.app-aware-link:eq(1)").attr('href') != 'undefined') {
						...
						try {
							...
							var url =  $(t).find("a.app-aware-link:eq(1)").addClass('tech44444444').attr('href')
						console.log(url)
							if (url.indexOf('headless') == -1) {
						// 	console.log(url)


							var username = url.replace(/\/in\/|\//gi, "");

							var fullname = $(t).find("a.app-aware-link:eq(1) span:eq(0)").text();

							var fname = fullname.split(" ")[0];
							var lname = fullname.split(" ")[1];
							var title = $(t).find(".entity-result__primary-subtitle").text();
						...
						var url =  $(t).find("a.app-aware-link:eq(1)").addClass('tech44444444').attr('href') + "?lacontrol=endrose";
							...
							$(_class).eq(i).text("Profile Viewed");
							$(_class).eq(i).css({
								color: "grey",
								fontSize: "14px"
							});
			...
			// Auto Profile Follow
			if (s.feature == 4 && $(_class).length) {
				var t = $(".search-result.search-result__occluded-item").eq(i)[0];
				var url = "https://www.linkedin.com" + t.getElementsByTagName("a")[0].getAttribute("href");

				if ($("#linkedassist_iframe").length) {
					$("#linkedassist_iframe").remove();
				}
				...
				d.onload = function () {
					...
					setInterval(function () {
						$(e).find("body").css("zoom", ".1");
						$(e).find("body").scrollTop($(e).find("body")[0].scrollHeight + 300);

						$(e).find('.pv-s-profile-actions__overflow-toggle.button-secondary-large-muted').eq(0).trigger("click");
						$(e).find('.pv-s-profile-actions.pv-s-profile-actions--follow').eq(0).trigger("click");

					}, 1);
				}


				$(_class).eq(i).text("Profile Followed");
				$(_class).eq(i).css({
					color: "grey",
					fontSize: "14px"
				});
				...
				chrome.storage.local.get("downloadedCsv", function (e) {
					...
					if (d && typeof d != "undefined") {
						...
						$if.on("load", function () {
							...
							var st11 = setInterval(function () {
								var message_btn = $b.find(".pv-s-profile-actions.pv-s-profile-actions--message");
								if (message_btn.length) {
									...
									setTimeout(function () {
										try {
											...
											var t = $b.find(".msg-overlay-conversation-bubble").not(".msg-overlay-conversation-bubble--is-minimized");
				...
				chrome.storage.local.get("requests_data", function (e) {
					...
					if (d && typeof d != "undefined") {
						...
						if ($if[0].contentDocument.readyState == "complete") {
							...
							var c = $if.contents().find(".mn-invitation-card.mn-person-card.mn-person-card--selectable.ember-view");

							$(c).each(function () {
								var currentc = $(this);

								var username = currentc.find(".mn-person-info__picture.mn-person-info__picture--selectable").attr("href").replace(/\/in\/|\//gi, "");
								var ret = true;
								...
								$.each(d, function (y, x) {
									if (x.username == username) {
										...
										var diff = ((((ntime - time) / 1000) / 60) / 60);
										//console.log(diff+" "+s.note);
										if (diff > s.note) {

											currentc.find(".mn-person-card__person-btn-ext.button-tertiary-medium-muted:eq(0)").trigger("click");

											var uu = new Array();
											var ld = localStorage.delete_requests;
			...
			if (s.feature == 9 && !window.location.href.match(/lacontrol=connection/gi)) {
				...
				var addSuggested = function () {
					$body = $("#addSuggestedIframe").contents();
					$body.scrollTop(40000);

					//var e = $body.find("button[data-control-name='invite']").not("[LA='true']");
					var e = $body.find("a[data-control-name='pymk_profile']").not("[LA='true']").eq(i);

					e.attr("LA", true);
					//  e.eq(0).trigger("click");
					if (e.length) {
						var link = "https://www.linkedin.com" + e.attr("href") + "?lacontrol=connection";
			...
			if (s.feature == 10) {
				...
				if (!window.location.href.match(/lacontrol=connection/gi)) {
					if (!$('.feed-base-likers-modal.feed-base-likers-modal--artdeco-variant.feed-fe-modal').length) {
						$("button[data-control-name='likes_count']").click();
						var e = $(".feed-base-likers-modal__actor-list.actor-list");
						setInterval(function () {
							e.scrollTop(e[0].scrollHeight * 99999999999);
						}, 1000);
					}

					var c = $(".actor-item").eq(i);

					var link = "https://www.linkedin.com" + c.find("a:eq(0)").attr("href") + "?lacontrol=connection";
			...
			if (s.feature == 11) {
				...
				if (!window.location.href.match(/lacontrol=connection/gi)) {
					...
					var item = $('.feed-shared-comments-list__comment-item.feed-shared-comment-item').not("[lac='true']");
					if (item.length) {
						...
						var c = item.eq(i).find("a:eq(0)");

						if (c.length) {
							c.attr("lac", true);
							var link = "https://www.linkedin.com" + c.attr("href") + "?lacontrol=connection";
					...
					if (s.pause == true) {
						...
						if (!window.location.href.match(/lacontrol=message/gi)) {
							...
							setTimeout(function () {
								try {
									$("html").animate({
										scrollTop: $(".message-anywhere-button").not("[lac='true']").eq(0).offset().top - 300
									}, 1500, function () {
										if ($(".message-anywhere-button").not("[lac='true']").eq(0).text().match(/Message|Nachricht/gi)) {
											...
											$(".message-anywhere-button").not("[lac='true']").eq(0).trigger("click");

											var t = $(".msg-overlay-conversation-bubble").not(".msg-overlay-conversation-bubble--is-minimized");
											var name = $(".message-anywhere-button").not("[lac='true']").eq(0).parents(".groups-members-list__typeahead-result").find(".artdeco-entity-lockup__title").text().trim();
													...
													if (t.prevObject[0].innerText.indexOf(n) === -1) {
														...
														if (!ttext.match(re)) {
															...
															setTimeout(function () {
																...
																var findCloseModal = setInterval(()=>{

																	if ($('.artdeco-modal__dismiss[aria-label="Dismiss"]').length > 0) {
																		clearInterval(findCloseModal);
																		$('.artdeco-modal__dismiss[aria-label="Dismiss"]').click()
																	}

																},10)

																$(".message-anywhere-button").not("[lac='true']").eq(0).text("Sent");
																$(".message-anywhere-button").not("[lac='true']").eq(0).css({
																	color: "grey",
																	fontSize: "14px"
																});
																$(".message-anywhere-button").not("[lac='true']").eq(0).attr("lac", true);
															}, 500);
														}else{
															//skipCountrer = true; 
															t.find("li-icon[type='cancel-icon']").parent().click();
			...
			if (s.feature == 12) {
				...
				var addSuggested = function () {
					...
					switch (s.custom) {
						...
						case "1": {
							button = $body.find("button").filter(":contains('Say happy birthday'),:contains('Gratulieren')");

							button = button.filter(function (x, y) {
								return $(y).parent().parent().parent().find(".nt-card__headline:eq(0) strong").text().match(/a happy birthday|alles Gute zum Geburtstag/gi);
							})
							break;
						}
						case "2": {
							button = $body.find("button").filter(":contains('Say congrats'),:contains('Gratulieren')");
						...
						case "3": {
							button = $body.find("button").filter(":contains('See all views'),:contains('Alle Ansichten anzeigen')");

							break;
						}
						case "4": {
							button = $body.find("button").filter(":contains('Say thanks'),:contains('Bedanken')");
					...
					if ($body.find(".me-wyva-card__title").length) {

						$body.find("button[data-control-name='connect_CTA']:eq(0)").trigger("click");
					}

					if (button.length) {
						console.log(button.eq(0)[0]);
						button.eq(0).attr("la", "true");

						$body.find("*[data-control-name='overlay.close_conversation_window']").each(function () {
							$(this).trigger("click");
						})
						...
						var t = $body.find(".msg-overlay-conversation-bubble").not(".msg-overlay-conversation-bubble--is-minimized");

						if (t.length) {
							var name = t.find(".msg-connections-lookup__recipient-name").text().replace(/\s+ /, "")
							name = name.split(" ")[0];
							...
							t.find(".ember-text-area.msg-messaging-form__message.ember-view").val(n);
							...
							t.find(".msg-messaging-form__send-button.button-tertiary-small").attr("disabled", false);

							t.find(".msg-messaging-form__send-button.button-tertiary-small").trigger("click");
						}
					}
				}


				if (!$("#congratesIframe").length) {
					$("body").append($if);
				} else {
					addSuggested();
				}
				...
				setTimeout(() => {
					...
					if (i >= 10 || is_next_available == 0) {
						...
						if (!page) {
							...
							var w = window.location.href.replace(/&page=\d/, "");
...
$(document).ready(function () {
	if (!window.location.href.match(/lacontrol=endrose/gi)) {
		...
		chrome.storage.local.get("settings", function (se) {
			...
			if (s) {
				if (s.pause == false) {
					var url = chrome.extension.getURL("src/browser_action/browser_action.html");
					...
					if (s.feature == "14") {
						prepare_s_14(s.num).then(() => {
							var timeInterval = (parseInt(s.time) * 1000);
...
$(document).ready(function () {
	...
	if (window.location.href.match(/lacontrol=connection/gi)) {
		setTimeout(function () {
			...
			if ($(".pv-s-profile-actions.pv-s-profile-actions--connect").length) {
				$(".pv-s-profile-actions.pv-s-profile-actions--connect:eq(0)").trigger("click");
				$(".send-invite__actions button:eq(0)").trigger("click");

				var name = $(".pv-top-card-section__name:eq(0)").text();
				name = name.split(" ")[0];
				...
				$(".button-primary-large").eq(1)[0].click();
...
function checkUsername() {
	var username = $('code').text().match(/"publicIdentifier"\:"\S+"\,/gi);
	...
	if (username) {
		...
		if (username.length > 1) {
			...
			if (user.username == "first") {
				...
				chrome.extension.sendMessage({ "msg": "updateUsername", username: username });
...
function clickClasses() {
	...
	$("html").animate({
		scrollTop: $(".pv-skill-entity__featured-endorse-button-shared").eq(0).offset().top - 300
	}, 2000, function () {
...
function activateEndorse() {
	...
	var EndorseCounter = 0;
	var EndorseInterval = setInterval(function () {
		EndorseCounter++;
		if ($(".pv-skill-entity__featured-endorse-button-shared").length) {
